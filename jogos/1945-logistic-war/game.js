...
class Game {
    ...
    setupEventListeners() {
        ...
        // MODIFICADO: Botão de reiniciar envia requisição ao pai
        ...
        // MODIFICADO: Botões de retry nos overlays enviam requisição ao pai
        this.retryBtns.forEach(btn => btn.addEventListener('click', () => {
            window.parent.postMessage({ type: 'RESTART_REQUEST' }, '*');
        }));
    ...
    // Apenas para inicialização primária (primeiro carregamento do Iframe)
    reset() {
        this.grid = Array.from({ length: GRID_SIZE }, () => Array(GRID_SIZE).fill(null));
        this.score = 0;
        this.won = false;
    ...
    move(direction) {
        ...
        for (const r of rowIndices) {
            for (const c of colIndices) {
                ...
                if (target && target.value === tile.value && !target.merged) {
                    this.score += Math.floor((tile.value * 2) / 3);
                    nextGrid[nr][nc] = { ...tile, value: tile.value * 2, r: nr, c: nc, merged: true, new: false };
                    moved = true;
                    if (tile.value * 2 === 2048 && !this.won) {
                        this.won = true;
                        shouldNotifyVictory = true;
                    }
                } else {
        ...
        if (moved) {
            ...
            // MODIFICADO: Notifica vitória imediatamente
            if (shouldNotifyVictory && !this.victoryMessageSent) {
                window.parent.postMessage({ type: 'GAME_OVER', score: this.score }, '*');
                this.victoryMessageSent = true;
            }
    ...
    checkGameOver() {
        ...
        for (let r = 0; r < GRID_SIZE; r++) {
            for (let c = 0; c < GRID_SIZE; c++) {
                ...
                if (r > 0 && this.grid[r - 1][c]?.value === v) movesAvailable = true;
                if (r < GRID_SIZE - 1 && this.grid[r + 1][c]?.value === v) movesAvailable = true;
                if (c > 0 && this.grid[r][c - 1]?.value === v) movesAvailable = true;
                if (c < GRID_SIZE - 1 && this.grid[r][c + 1]?.value === v) movesAvailable = true;
        ...
        if (!movesAvailable) {
            ...
            // MODIFICADO: Notifica derrota imediatamente enviando a pontuação
            window.parent.postMessage({ type: 'GAME_OVER', score: this.score }, '*');
        }
    }
    ...
    render() {
        ...
        for (let r = 0; r < GRID_SIZE; r++) {
            for (let c = 0; c < GRID_SIZE; c++) {
                ...
                void el.offsetWidth;
                if (data.new) el.classList.add('tile-new');
                if (data.merged) el.classList.add('tile-merged');
        ...
        for (const [id, el] of this.tileElements.entries()) {
            if (!currentIds.has(id)) { el.remove(); this.tileElements.delete(id); }
        }
    }
