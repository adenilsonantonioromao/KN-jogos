    ...
    render() {
        ...
        for (let r = 0; r < GRID_SIZE; r++) {
            for (let c = 0; c < GRID_SIZE; c++) {
                ...
                // Aplicar coordenadas e classes de animação
                el.style.transform = `translate(${pos.left}px, ${pos.top}px)`;
                
                // IMPORTANTE: Remove e adiciona as classes de estado para resetar animações CSS
                el.classList.remove('tile-new', 'tile-merged');
                void el.offsetWidth; // Force reflow para disparar animação novamente se necessário
